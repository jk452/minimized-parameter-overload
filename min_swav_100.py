...
Original file is located at
    https://colab.research.google.com/drive/17P771wm0stOmCNP7A6J59ttV6vxiccRA
"""
...
class Net(nn.Module):
    def __init__(self, trial):
        ...
        input_dim_cnn = 1024
        #cnn_layers = 1
        num_cnn_layers = trial.suggest_int("num_cnn_layers", 1, 2,step=1)

        for i in range(num_cnn_layers):
            output_dim_cnn = trial.suggest_int("n_units_cnn_l{}".format(i), 64, 512, step = 64)
            #output_dim_cnn = 512

            cnnlayers.append(nn.Conv2d(input_dim_cnn, output_dim_cnn, 2, stride=1, padding=1))
            cnnlayers.append(nn.BatchNorm2d(output_dim_cnn))
            cnnlayers.append(nn.ReLU(inplace=True))
            cnnlayers.append(nn.MaxPool2d(3, stride=1))
        ...
        for i in range(n_layers):
            output_dim = trial.suggest_int("n_units_l{}".format(i), 64, 1024, step = 64)
            #output_dim = 384
...
class LightningNet(pl.LightningModule):
    def __init__(self,trial,learning_rate):
        ...
        self.backbone = nn.Sequential(*(list(swav.children())[:-3]))
        #self.backbone = swav
...
CIFAR100_full.transform = transform_train
#CIFAR100_full.transform = transform_train
CIFAR100_test.transform = transform_test
train_dataloader = DataLoader(CIFAR100_full, batch_size=BATCHSIZE, shuffle=True)
#val_dataloader = DataLoader(CIFAR100_val, batch_size=BATCHSIZE, shuffle=False)
val_dataloader = DataLoader(CIFAR100_test, batch_size=BATCHSIZE, shuffle=False)
...
study_name = "swav_100_n"  # Unique identifier of the study.
...
print(df)
df.to_csv('swav_100_n_less.csv',index=False)

df_saved_files = pd.read_csv('swav_100_n_less.csv')
